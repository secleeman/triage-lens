...
スキャナが見つけた脆弱性を、**説明可能で再現可能な「対応判断」に変換する
オープンな判断エンジン** です。スキャナの検出結果と公開された脅威データを
入れると、根拠の付いたトリアージレポート（Markdown）が出てきます。
...
「判断エンジン」と名乗る以上、判断の中身を全部見せます。次の4つは仕様です。

- **判定は決定論的です。** 優先度 P0〜P3 は CISA KEV / EPSS / CVSS の値だけから
  機械的に決めます。同じ入力と同じ公開データからは、いつでも同じ結論が出ます。
  条件は表にして公開し、閾値はソースコードの1か所に集約しています
  （[優先度の付け方](#優先度の付け方)）。
- **AI は判断しません。** `--ai` を付けたときに AI が書くのは、決まった優先順位に
  対する「で、何をすればいいのか」の説明文だけです。順位を決めさせたり、
  書き換えさせたりはしません（[優先順位は AI が決めるのではありません](#優先順位は-ai-が決めるのではありません)）。
- **到達性は見ません。** 見ているのは「その版が依存関係に含まれているか」だけで、
  脆弱なコードが実際に呼ばれるかは判定しません。できないことを、できるとは書きません
  （[このレポートで分かることの限界](#このレポートで分かることの限界)）。
- **分からない入力では判断しません。** 外部データを取得できなかった値や、入力から
  読み取れなかった値は、「低い」ではなく **「不明」** と書きます。修正版の有無も
  「修正版なし」と「不明」を区別します（[「修正版なし」と「不明」の違い](#修正版なしと不明の違い)）。

判定に使う公開データ（CISA KEV / EPSS / CVSS）はいずれも認証キーが不要で、
...
**Issue には `secleeman` として返信します。** ただし、**対応するかどうか、いつ対応するかは
約束できません。** 個人が空き時間で保守しているツールなので、返信が遅れることも、
「対応しない」と返すこともあります。対応した結果は、コミットとリリースノート
（[ROADMAP](https://github.com/secleeman/triage-lens/blob/main/docs/ROADMAP.md)）にも反映されます。

**脆弱性の報告は Issue ではなく非公開の窓口へお願いします。** 手順は
[SECURITY.md](https://github.com/secleeman/triage-lens/blob/main/SECURITY.md) に書いています。
