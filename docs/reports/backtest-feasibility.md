...
**⚠️ 行の並び順は保証されていません**（実測）。**並び順に依存した処理を書かないでください。**

ファイルごとに先頭行も並びも違います。EPSS 降順でも CVE ID 昇順でもありません。

| ファイル | 最初のデータ行 | EPSS降順の違反数 | CVE ID昇順の違反数 |
| --- | --- | --- | --- |
| 2023-01-01（v2） | `CVE-2020-14882,0.94926` | 59,969 | 7,961 |
| 2023-03-07（v3 初日） | `CVE-2023-0001,4.3e-4` | 83,627 | 7,202 |
| 2024-01-01（v3） | `CVE-1999-0001,0.01167` | 93,486 | 9,874 |
| 2025-01-01（v3） | `CVE-1999-0001,0.00383` | 105,675 | 61 |

**⚠️ スコアの表記も揺れます。** v3 の公開初日（2023-03-07）は 196,955行中
**77,381行が `4.3e-4` のような指数表記**でした。一方 2023-01-01 / 2024-01-01 /
2025-01-01 のファイルには指数表記が1行もありません。
**文字列としてではなく必ず float に変換して比較してください。**
